...
*/
    ...
    $finbest_footer_logo = get_theme_mod( 'finbest_footer_logo' );
    $finbest_footer_top_space = function_exists('tpmeta_field') ? tpmeta_field('finbest_footer_top_space') : '0';
    $finbest_copyright_center = $finbest_footer_logo ? 'col-lg-4 offset-lg-4 col-md-6 text-right' : 'col-lg-12 text-center';
    //footer bg image & color
    $finbest_footer_bg_url_from_page = function_exists( 'tpmeta_image_field' ) ? tpmeta_image_field( 'finbest_footer_bg_image' ) : '';

    $finbest_footer_bg_color_from_page = function_exists( 'tpmeta_field' ) ? tpmeta_field( 'finbest_footer_bg_color' ) : '';
    ...
    // bg image
    $bg_img = !empty( $finbest_footer_bg_url_from_page['url'] ) ? $finbest_footer_bg_url_from_page['url'] : $footer_bg_img;

    // bg color
    $bg_color = !empty( $finbest_footer_bg_color_from_page ) ? $finbest_footer_bg_color_from_page : $footer_bg_color;
    // Email id 
   $header_top_email = get_theme_mod( 'header_email', __( 'finbest@support.com', 'finbest' ) );

    // Phone Number
   $header_top_phone = get_theme_mod( 'header_phone', __( '+88 01310-069824', 'finbest' ) );
   $footer_bottom_menu = get_theme_mod( 'footer_bottom_menu', __( '#', 'finbest' ) );


    // footer area links  switch
    $footer_area_links_switch = get_theme_mod( 'footer_area_links_switch', false );
    // footer area links 
    $footer_area_links = get_theme_mod( 'footer_area_links', __( '#', 'finbest' ) );
            ...
            <div class="row">
                <div class="col-md-12 col-lg-6">
                    <div class="tp-footer-copyright-inner">
                        <p><?php print finbest_copyright_text(); ?></p>
                    </div>
                </div>
                <div class="col-md-12 col-lg-6">
                    <div class="tp-footer-copyright-inner text-lg-end">
                        <?php echo finbest_kses( $footer_bottom_menu ); ?>
                    </div>
                </div>
            </div>
        </div>
