...
if( !function_exists('finbest_content_single_details') ) {
    function finbest_content_single_details( ) {
        global $product;
        global $post;
        global $woocommerce;
    ...
    <div class="tp-shop-details__right-warp">
        ...
        <?php if(!empty($rating)) : ?>
            <div class="tp-shop-details__ratting d-flex">
                ...
                <span class="review-text">( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'finbest'); echo esc_html($ratingcount) <= 1 ? 'review' : 'reviews'; ?> )</span>
            </div>
            <?php else : ?>
            <div class="tp-shop-details__ratting">
                ...
                <span><i class="fal fa-star"></i></span>
                <span><i class="fal fa-star"></i></span>
                <span>( <?php echo esc_html($ratingcount); ?> <?php echo esc_html__('customer ', 'finbest'); echo esc_html($ratingcount) <= 1 ? 'review' : 'reviews'; ?> )</span>
            </div>
        <?php endif; ?> 
...
add_action( 'woocommerce_single_product_summary', 'finbest_content_single_details', 4 );
...
*************************************************/
function finbest_shipping_progress_bar() {
...
if(get_theme_mod( 'shipping_progress_bar_location_card_page',0) == '1'){
    add_action( 'woocommerce_before_cart_table',  'finbest_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_mini_cart',0) == '1'){
    add_action( 'woocommerce_before_mini_cart_contents', 'finbest_shipping_progress_bar' );
}

if(get_theme_mod( 'shipping_progress_bar_location_checkout',0) == '1'){
    add_action( 'woocommerce_checkout_before_customer_details', 'finbest_shipping_progress_bar' );
}
...
function finbest_sale_percentage(){
   global $product;
   $output = '';
...
if ( ! function_exists( 'finbest_header_add_to_cart_fragment' ) ) {
    function finbest_header_add_to_cart_fragment( $fragments ) {
        ob_start();
        ?>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'finbest_header_add_to_cart_fragment' );


// product-content archive
if( !function_exists('finbest_content_product_grid') ) {
    function finbest_content_product_grid( ) {
    global $product;
    global $post;
    global $woocommerce;
    $rating = wc_get_rating_html($product->get_average_rating());
        ...
        <div class="product__thumb w-img p-relative fix">
            ...
            <div class="product__action d-flex flex-column flex-wrap">
                ...
                <div class="product-action-btn product-add-wishlist-btn p-relative">
                    ...
                    <span class="product-action-tooltip"><?php echo esc_html__('Add To Wishlist','finbest'); ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="product-action-btn p-relative">
                    ...
                    <span class="product-action-tooltip"><?php echo esc_html__('Quick view','finbest'); ?></span>
                </div>
                <?php endif; ?>
                ...
                <div class="product-action-btn p-relative">
                    ...
                    <span class="product-action-tooltip"> <?php echo esc_html__('Add To Compare','finbest'); ?></span>
                </div>
                <?php endif; ?>
...
add_action( 'woocommerce_before_shop_loop_item', 'finbest_content_product_grid', 10 );


// smart quickview
add_filter( 'woosq_button_html', 'finbest_woosq_button_html', 10, 2 );
function finbest_woosq_button_html( $output , $prodid ) {
    ...
    $price_class = $price ? NULL : 'price-empty';
        ...
        if ( $product ) {
         ...
         if( $product->is_type( 'simple' ) ){
            $btntext = esc_html__("Add to Cart",'finbest');
         } elseif( $product->is_type( 'variable' ) ){
            $btntext = esc_html__("Select Options",'finbest');
         } elseif( $product->is_type( 'external' ) ){
            $btntext = esc_html__("Buy Now",'finbest');
         } elseif( $product->is_type( 'grouped' ) ){
            $btntext = esc_html__("View Products",'finbest');
         }
         else{
            $btntext = "Add to Cart";
         } 
        ...
        $args = wp_parse_args(
            ...
            apply_filters(
                ...
                array(
                    ...
                    'before'      => '',
                    'after'       => '',
                    'home'        => _x( 'Home', 'breadcrumb', 'finbest' ),
                )
            )
        );
...
/*************************************************
## finbest Woo Search Form
*************************************************/ 

add_filter( 'get_product_search_form' , 'finbest_custom_product_searchform' );

function finbest_custom_product_searchform( $form ) {

	$form = '<div class="search-form p-relative">
                <form action="' . esc_url( home_url( '/shop'  ) ) . '" role="search" method="get" id="searchform">
                <input type="search" value="' . get_search_query() . '" name="s" placeholder="'.esc_attr__('Search Product','finbest').'" autocomplete="off">
